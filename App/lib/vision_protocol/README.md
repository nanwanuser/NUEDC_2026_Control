...
视觉上报的坐标采用横放 A4 纸左上角为原点，沿长边向右为 `+X`，纸面内向下为
`+Y`，单位 mm；`X` 范围为 `0..297 mm`，`Y` 范围为 `0..210 mm`，左右半区
分界为 `X = 148.5 mm`。决策层直接在这个坐标系里求解并输出摆放位置，中间不做任何
坐标变换，因此视觉端换错坐标系不会报错，只会让机构走到错误的位置。
...
确认后 STM32 立即冻结 3 帧平均值，调用 `HAL_UART_AbortReceive()` 停止本轮接收，
随后直接调用 `DecisionTask_Submit()`。USART1 保持初始化，但本轮不再解析后续串口
数据；下一次按键重新调用 `VisionUart_Arm()` 后才重新启动接收。STM32 不向视觉端
发送 ACK 或其他数据。
...
每次 `VisionUart_Arm()` 都会清空本轮接收状态并重新启动 `ReceiveToIdle`，所以一轮
提交后仍可再次按键采集，不需要重新上电。`VisionUart_Abort()` 放弃当前采集。
